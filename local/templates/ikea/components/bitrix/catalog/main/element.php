...
$APPLICATION->SetPageProperty("SHOW_RECOMMENDS", "N");
...
<div class="product-card_section">
	<div class="container">
        ...
        if (!empty($arResult['VARIABLES']['ELEMENT_ID']) || !empty($arResult['VARIABLES']['ELEMENT_CODE'])) {
            ...
            if($ob = $res->Fetch()){
                $list = CIBlockSection::GetNavChain(false, $ob['IBLOCK_SECTION_ID'], array());
                while ($arSection = $list->GetNext()) {
        ...
        ?>
        <? if (!$GLOBALS['relatedFilter']['=ID']) {
            $GLOBALS['relatedFilter']['ID'] = false;
        } ?>
		<div class="product-added_fixed-box">
			<div class="product-added_header">
				<span class="title">Добавили <span class="product-added_name">товар</span> в вашу корзину</span>
				<a href="<?= $arParams["BASKET_URL"] ?>" class="white-btn">Перейти в корзину</a>
				<span class="product-added_close-btn ico-close"></span>
			</div>
            <? $APPLICATION->IncludeComponent(
...
<? $this->SetViewTarget('same_goods'); ?>
<? $GLOBALS['similarFilter']['!ID'] = $elementId ?>
<? $APPLICATION->IncludeComponent(
    "bitrix:catalog.section",
    "similar",
    array(
