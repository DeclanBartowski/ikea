...
<? else: ?>
	...
	<div id="sls-<?= $arResult['RANDOM_TAG'] ?>"
	     class="bx-sls form-control <? if ($arResult['MODE_CLASSES'] <> ''): ?> <?= $arResult['MODE_CLASSES'] ?><? endif ?>">
		...
		<div class="<?= $dropDownBlock ?> bx-ui-sls-input-block">
			...
			<input type="text" autocomplete="off" name="<?= $arParams['INPUT_NAME'] ?>"
			       value="<?= $arResult['VALUE'] ?>" class="dropdown-field"
			       placeholder="<?= Loc::getMessage('SALE_SLS_INPUT_SOME') ?> ..."/>

			<div class="dropdown-fade2white"></div>
			<div class="bx-ui-sls-loader"></div>
			<div class="bx-ui-sls-clear input_delete-text ico-close is-visible"
			     title="<?= Loc::getMessage('SALE_SLS_CLEAR_SELECTION') ?>"></div>
